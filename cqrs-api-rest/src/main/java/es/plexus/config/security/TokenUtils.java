...
public class TokenUtils {
    ...
    public static String createToken(UserDetailsImpl userDetails) {
        ...
        extra.put("surnames", userDetails.getSurnames());
        extra.put("rol", userDetails.getRolName());
    ...
    public void verifyTokenByUserId(String token, long userId){
        if (userId != Long.valueOf(extractClaim(token, c -> c.get("id")).toString()) && !("admin".equals(extractClaim(token, c -> c.get("rol")).toString()))){ //Comprobar si es admin cunado esten los roles
